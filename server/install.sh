...
if [[ -z "$MODULES_DIR" ]]; then
  for d in "$HOME"/.local/share/Logos/LogosBasecamp*/modules; do
    [[ -d "$d/mesh_gateway" ]] && MODULES_DIR="$d" && break
  done
fi
...
[[ -n "$MODULES_DIR" && -d "$MODULES_DIR/mesh_gateway" ]] || {
  echo "ERROR: mesh_gateway not found under a modules dir; set MODULES_DIR=..." >&2; exit 1; }
...
install -m 0755 "$here/mesh-gateway-run" "$HOME/.local/bin/mesh-gateway-run"
install -m 0755 "$here/gwctl"                  "$HOME/.local/bin/gwctl"

cat > "$HOME/.config/mesh-gateway.env" <<EOF
LOGOSCORE=$LOGOSCORE
MODULES_DIR=$MODULES_DIR
EOF

install -m 0644 "$here/mesh-gateway.service" "$HOME/.config/systemd/user/mesh-gateway.service"

# ---- enable + start ----------------------------------------------------------
systemctl --user daemon-reload
systemctl --user enable mesh-gateway.service
...
echo "       systemctl --user stop mesh-gateway"
echo "       gwctl channels                 # see indices"
echo "       gwctl relay 1 on               # bridge channel 1, etc."
echo "  2. Start it:"
echo "       systemctl --user start mesh-gateway"
echo "       journalctl --user -u mesh-gateway -f"
